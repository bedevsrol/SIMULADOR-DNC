function soxMora(){    
  ...
  let descIntExtracontable = getFieldValue('0456eeb3-8809-48a5-8726-87e416efdcb3') || '0';
...
`TITULAR TOMA PAGO MORA DE CONTADO PARA EL PRODUCTO ${producto} CUYO PAGO MINIMO A LA FECHA ES DE ${pagoMinimo} DE LOS CUALES SE LE GENERARA UNA BAJA EN CUENTA DE INTERES MORATORIO DEL ${descIntMora}%, DE INTERES CORRIENTE DE ${descIntCte}%, DE INTERES EXTRACONTABLE DE ${descIntExtracontable}% Y BAJA EN CUENTA TOTAL DE ${descTotal} DANDO COMO ABONO A PAGAR UN MONTO DE ${pagoSNR} EFECTIVO OFICINAS BANCO DE BOGOTA Y AL SNR SE LE INFORMA DE POLITICAS Y CONDICIONES ESTA DE ACUERDO. SE LE INFORMA NEGOCIACION SUJETA A VERIFICACION, APROBACION O NEGACION POR PARTE DEL BANCO. SE RECUERDA BLOQUEO DE PRODUCTOS CON CUPO POR 120 DIAS`;

  setFieldValue('96c93177-4705-4bd2-ac50-e304c007afa3', observacionesBase);

  // Llenar SOX inicial
  const sox =`FECHAPAGOXX${fechaPago}LLLVALORCONSIGSNRXX${pagoSNR}LLLVALORPAGOPRODUCTOXX0LLLVALORHONORARIOSXX0LLLVALORPRODUCTOXX${pagoSNR}LLLTIPONEGXXTELLLLCUOTAPROYECTADAXXNO APLICALLLOBSERVACIONESXX${observacionesBase}LLLEXCEPCIONXX
  ${excepcion}LLLCUOTAVENCIDAXX${cuotaVencida}LLL`;
