//nombre en lappozllenar campos observaciones novacion sox
function llenarCampos() {
    ...
    let observaciones = `TITULAR TOMA NOVACION DE SALDO TOTAL PARA SU TC ${obligacion} 
    CUYO SALDO TOTAL ES DE $${saldoDiferir} ESTO DISPUESTO A UN PLAZO DE ${plazo} MESES,
    CON ${tasa} NMV QUE LE GENERA UNA CUOTA APROXIMADA POR $${cuotaProyectada} SIN INCLUIR
    CUOTA DE MANEJO SEGURO O NUEVOS USOS. ACEPTA NOVACION CON ABONO DE $${pagoNegociacion} 
    REALIZADO EL ${fechaPago} LA ALTERNATIVA QUEDA SUJETA A VERIFICACION, APROBACION O NEGACION POR PARTE DEL BANCO.`;

    let plantillaSOX = `FECHAPAGOXX${fechaPago2}LLLCOMPRASAUTXX${comprasAut}LLLPLAZOAUTXX${plazoAut}
                        LLLCODEXCXX${codigoExp}LLLOCUPACIONXX${actividadEconomica}LLLSALARIOPENSIONOINGRESOBRUTOXX${ingresoBruto}
                        LLINGRESOSADICIONALESXX${ocupacionIngresosAd}LLLOCUPACIONINGRESOS ADICIONALESXX${ingresosAd}
                        LLLCUOTAS MENSUALESSECTOR FINANCIEROXX${coutaBDB}LLLMARCACR026XX${marca}LLLOBSERVACIONESXX${observaciones}LLL`
